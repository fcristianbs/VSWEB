import os, cv2, zipfile, numpy as np, threading, queue, shutil
import requests, re
    ...
    with zipfile.ZipFile(caminho_zip, 'r') as z:
        for file_info in z.infolist():
            if not file_info.is_dir() and file_info.filename.lower().endswith(('.jpg', '.jpeg', '.png')):
                ...
                caminho_orig = os.path.join(app.config['UPLOAD_FOLDER'], nome_orig)
...
@app.route('/api/carregar_projeto/<projeto_id>')
def carregar_projeto(projeto_id):
    try:
        ...
        for doc in projeto.documentos:
            ...
            if orig:
                orig_drive = os.path.join(app.config['DRIVE_FOLDER'], orig)
                orig_up = os.path.join(app.config['UPLOAD_FOLDER'], orig)
                if os.path.exists(orig_drive) and not os.path.exists(orig_up): shutil.copy(orig_drive, orig_up)
            
            if proc:
                proc_drive = os.path.join(app.config['DRIVE_FOLDER'], proc)
                proc_up = os.path.join(app.config['UPLOAD_FOLDER'], proc)
...
@app.route('/api/runrunit/obras_recentes')
...
@app.route('/buscar_gpm', methods=['POST'])
def buscar_gpm():
    dados = request.json
    def notify(msg): log_queue.put(msg)
    motor = MotorGPM(dados['user'], dados['pass'], dados['codigo'], app.config['DOWNLOADS_GPM'], notify)
    threading.Thread(target=motor.rodar).start()
    return jsonify({'status': 'ok'})

@app.route('/progresso_gpm')
def progresso_gpm():
    def generate():
        while True:
            msg = log_queue.get()
...
@app.route('/processar_download_gpm', methods=['POST'])
def processar_download_gpm():
    try:
        arquivos = [f for f in os.listdir(app.config['DOWNLOADS_GPM']) if f.endswith('.zip')]
        if not arquivos: return jsonify({'erro': 'ZIP não encontrado'}), 400
        arquivos.sort(key=lambda x: os.path.getmtime(os.path.join(app.config['DOWNLOADS_GPM'], x)), reverse=True)
        return jsonify(extrair_zip_apenas(os.path.join(app.config['DOWNLOADS_GPM'], arquivos[0])))
...
def upload():
    ...
    for f in files:
        ...
        path_orig = os.path.join(app.config['UPLOAD_FOLDER'], nome_orig)
        f.save(path_orig)
...
@app.route('/processar_imagem_unica', methods=['POST'])
...
@app.route('/rotacionar_apenas', methods=['POST'])
def rotacionar_apenas():
    dados = request.json
    id_i, ang = dados['id'], dados.get('angulo', 0)
    
    nome_proc = db_imagens[id_i]['processada']
    path_proc = os.path.join(app.config['UPLOAD_FOLDER'], nome_proc)
...
@app.route('/reajustar', methods=['POST'])
def reajustar():
    dados = request.json
    id_i, pts = dados['id'], np.array(dados['pontos'], dtype="float32")
    
    path_orig = os.path.join(app.config['UPLOAD_FOLDER'], db_imagens[id_i]['original'])
...
def salvar_documento():
    ...
    for id_img in dados['ids']:
        if id_img in db_imagens:
            caminho_arquivo = os.path.join(app.config['UPLOAD_FOLDER'], db_imagens[id_img]['processada'])
    ...
    path = os.path.join(app.config['UPLOAD_FOLDER'], f"{uuid4().hex[:5]}_{nome}")
...
@app.route('/salvar_nuvem', methods=['POST'])
def salvar_nuvem():
    dados = request.json
    ...
    try:
        ...
        for doc_front in documentos:
            ...
            if doc_db:
                ...
                if id_img in db_imagens:
                    caminho_temp_proc = os.path.join(app.config['UPLOAD_FOLDER'], db_imagens[id_img]['processada'])
                    caminho_def_proc = os.path.join(app.config['DRIVE_FOLDER'], doc_db.caminho_cortado)
                ...
                if id_img in db_imagens:
                    ...
                    caminho_temp_orig = os.path.join(app.config['UPLOAD_FOLDER'], nome_orig)
                    caminho_temp_proc = os.path.join(app.config['UPLOAD_FOLDER'], nome_proc)
                    
                    if os.path.exists(caminho_temp_orig): shutil.copy(caminho_temp_orig, os.path.join(app.config['DRIVE_FOLDER'], novo_nome_orig))
                    if os.path.exists(caminho_temp_proc): shutil.copy(caminho_temp_proc, os.path.join(app.config['DRIVE_FOLDER'], novo_nome_proc))
...
@app.route('/api/excluir_projeto/<projeto_id>', methods=['DELETE'])
def excluir_projeto(projeto_id):
    try:
        ...
        for doc in projeto.documentos:
            caminho_orig = os.path.join(app.config['DRIVE_FOLDER'], doc.caminho_original)
            caminho_proc = os.path.join(app.config['DRIVE_FOLDER'], doc.caminho_cortado)
...
@app.route('/api/adicionar_fila', methods=['POST'])
def adicionar_fila():
    dados = request.json
...
@app.route('/api/status_fila', methods=['GET'])
def status_fila():
...
@app.route('/api/limpar_fila', methods=['DELETE'])
def limpar_fila():
    try:
...
@app.route('/api/priorizar', methods=['POST'])
def priorizar():
    dados = request.json
    codigo = dados.get('codigo')
...
@app.route('/api/status_robo', methods=['GET'])
def status_robo():
...
@app.route('/api/toggle_robo', methods=['POST'])
def toggle_robo():
    config = ConfiguracaoSistema.query.filter_by(chave='status_robo').first()
    if not config:
